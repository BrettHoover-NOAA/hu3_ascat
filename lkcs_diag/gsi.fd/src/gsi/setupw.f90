...
!    also repoted using geopotential height.
     ...
     if (z_height .or. sfc_data) then
        ...
        dpres = data(ihgt,i)
        dstn = data(ielev,i)
!       BTH: If HU-3 ASCAT data (Type=290,Subtype=6), we are
!            passing likelihood to this routine through the
!            data(ielev,i) slot. Reassign to zero (default 
!            for ASCAT data)
        if (itype==290 .and. icsubtype(ikx)==6) then ! HU-3 ASCAT
            dstn=0
        end if
