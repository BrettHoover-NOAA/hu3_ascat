...
!   2019-03-27  h. liu   - add abi
!   2022-05-06 Hoover   - add HU-3 ASCAT wind read via read_hu3ascat
!   
!
!   input argument list:
!     mype     - mpi task id
...
!             Process oscat winds which seperate from prepbufr
                elseif ( index(infile,'oscatbufr') /=0 ) then
                  call read_sfcwnd(nread,npuse,nouse,infile,obstype,lunout,gstime,twind,sis,&
                     prsl_full,nobs_sub1(1,i))
                  string='READ_SFCWND'
!             Process HU-3 ASCAT winds which are separate from prepbufr
                elseif ( index(infile,'hu3ascatbufr') /=0 ) then
                  call read_hu3ascat(nread,npuse,nouse,infile,obstype,lunout,gstime,twind,sis,&
                     prsl_full,nobs_sub1(1,i))
                  string='READ_HU3ASCAT'
